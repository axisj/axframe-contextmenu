...
class PopupMenu extends React.Component<IREWMenu.IPopupMenuProps, IREWMenu.IPopupMenuState> {
  state = {
    visible: true,
    positioned: false,
    newLeft: 0,
  ...
  static getDerivedStateFromProps(props: IREWMenu.IPopupMenuProps, state: IREWMenu.IPopupMenuState) {
    let nextState = state;
  ...
  align = () => {
    ...
    if (!this.state.positioned && this.popupMenuRef.current) {
      ...
      const marginTop = Number(
        ('' + window.getComputedStyle(this.popupMenuRef.current).marginTop).replace(/[a-zA-Z]/g, ''),
      );
  ...
  render() {
    ...
    return (
      <div className='rf-contextmenu' style={menuStyles} ref={this.popupMenuRef}>
